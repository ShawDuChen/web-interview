...
## CSS
  ...
  2. 伪类和伪元素的差别？  
    * 伪类：  
    ...
    2. css3中，css伪类存在的意义是为了通过选择器找到那些不存在与dom树中的信息以及不能被常规css选择器获取的信息；伪类由一个冒号(:)开头，冒号后面是伪类的名称和包含在圆括号中的可选参数；任何常规选择器可以在任何位置使用伪类；伪类语法不区分大小写。一些伪类的作用会互斥，另外一些伪类可以同时被同一个元素使用。并且，为了满足用户在操作dom时产生的DOM结构改变，伪类也可以是动态的。  
    * 伪元素：  
...
19. 普通函数和箭头函数的区别？
    1. 普通函数：可以通过bind、call、apply改变this的指向；可以使用new；
    2. 箭头函数：本身没有this指向；它的this继承自外层第一个普通函数的this；被继承的普通函数的this指向改变时，箭头函数的this指向会跟着改变；箭头函数外层没有普通函数时，this指向window；不能通过bind、call、apply改变this的指向；使用new调用箭头函数时会报错，因为箭头函数没有constructor；箭头函数不能绑定arguments，要用rest参数解决；
20. 内存泄漏的可能性？
    1. 意外的全局变量；
    2. 闭包；
    3. 没有清理的dom元素，dom元素赋值给变量，有通过removeChild移除dom元素，但dom元素的引用还在内存中；
    4. 被遗忘的定时器或回调；
21. new操作符做了什么？
    1. 创建一个空对象；
...
## 前端安全
  1. CSRF（跨站请求伪造）：是一种挟制用户在当前浏览器已登录网站上执行非本意的操作的攻击方式；  
    * 特点：（1）CSRF通常发生在第三方域名。（2）CSRF攻击者只能使用cookie而不能获取cookie；  
